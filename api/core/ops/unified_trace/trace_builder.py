...
class CanonicalTraceBuilder:
    ...
    def _build_workflow(self, trace_info: WorkflowTraceInfo) -> CanonicalTrace:
        ...
        workflow_start = _started_at(getattr(workflow_data, "created_at", None) or trace_info.start_time)
        workflow_end = getattr(workflow_data, "finished_at", None) or trace_info.end_time
        ...
        for item_execution_id, item in execution_by_id.items():
            process_data = getattr(item, "process_data", None) or {}
            outputs = getattr(item, "outputs", None) or {}
            node_type = str(getattr(item, "node_type", ""))
            error = getattr(item, "error", None)
            started_at = _started_at(getattr(item, "created_at", None))
            elapsed_time = getattr(item, "elapsed_time", None) or 0
            ...
            spans[item_execution_id] = CanonicalSpan(
                ...
                inputs=process_data.get("prompts", []) if node_type == "llm" else getattr(item, "inputs", None) or {},
                outputs=outputs,
                status=_status(error, getattr(item, "status", None)),
                error=error,
                metadata=metadata,
