...
def execution_id(execution: WorkflowExecutionLike) -> str:
    ...
    return str(getattr(execution, "id", None) or execution.node_execution_id)


def execution_metadata(execution: object) -> Mapping[str, Any]:
    """Read metadata from repository models and older trace test doubles."""
    value = getattr(execution, "execution_metadata_dict", None)
    if not isinstance(value, Mapping):
        value = getattr(execution, "metadata", None)
    if not isinstance(value, Mapping):
        return {}
...
def _metadata_or_attr(execution: object, metadata: Mapping[str, Any], key: str) -> Any:
    ...
    return value if value is not None else getattr(execution, key, None)
...
def _finished_at(execution: WorkflowExecutionLike) -> datetime:
    explicit_end = getattr(execution, "end_time", None)
    if isinstance(explicit_end, datetime):
        return explicit_end
    started_at = execution.created_at or getattr(execution, "start_time", None) or datetime.now()
