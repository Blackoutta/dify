...
def test_workflow_finish_response_does_not_require_persisted_workflow_run():
    ...
    converter = WorkflowResponseConverter(application_generate_entity=SimpleNamespace())
    ...
    assert response.data.workflow_id == "workflow-1"
    assert response.data.outputs == {"answer": "ok"}
    assert response.data.created_by is None
    assert response.data.total_tokens == 3
