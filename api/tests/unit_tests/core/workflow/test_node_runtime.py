...
def test_dify_tool_node_runtime_injects_outer_workflow_run_id_for_workflow_tools(
    ...
    monkeypatch.setattr(
        ...
        "get_system_text",
        lambda _pool, key: (
            "outer-workflow-run-id"
            if key == node_runtime.SystemVariableKey.WORKFLOW_EXECUTION_ID
            else None
        ),
    )
    ...
    assert runtime_tool.runtime.runtime_parameters["outer_workflow_run_id"] == "outer-workflow-run-id"
