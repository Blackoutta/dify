...
**Goal:** Make the default trace-parent retry window cover Dify's default maximum workflow execution time.
...
- [ ] Run the test and verify it fails with the current 300-second retry window.

```bash
uv run --project api pytest api/tests/unit_tests/tasks/test_ops_trace_task.py::test_process_trace_tasks_default_retry_window_covers_max_workflow_execution_time -q
```

- [ ] Change the default and environment examples from 60 to 240. Add this comment above the settings fields:
